...
process process_reads {
    ...
    output:
        tuple val(meta), path("${processed_fastq}"), path("${summary_stats}"), path("${kde_stats}"), path(stats)
    script:
        summary_stats = "${meta.alias}_telomere_unaligned_metrics.tsv";
        processed_fastq = "${meta.alias}_filtered_telomeric.fastq"
        kde_stats = "${meta.alias}_kde_data.tsv";
        """
        {
        ...
        | workflow-glue process_reads --summary-tsv-name ${summary_stats} \\
            --kde-tsv-name "${kde_stats}" "${meta.alias}" - \\
